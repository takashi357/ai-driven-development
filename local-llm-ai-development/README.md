...
日本の一般利用者・小規模事業者に向けた、ローカルLLMの使いどころを考えるWebアプリです。樋口隆史が目的・制約を提示し、診断結果の偏りを指摘。AIが実装・資料確認・操作検証を行い、診断の設計と検証方法を見直しました。[全体の役割分担](../README.md)も参照してください。
...
開発記録によると、最初の版は操作に応じて文章が変わる一方、16GBの54条件中39条件でモデルの出発点が4B級に偏っていました。樋口隆史から、選び方を変えても同じ答えに見えるとの指摘があり、AIが主な結論・モデル候補・検証の観点を見直しました。

| 段階 | 変更と学び |
|---|---|
| 条件診断の作成 | 108通りの入力に対して表示が出ることを確認 |
| 樋口の評価 | 画面が動くことと、判断に役立つことの差が明らかに |
...
詳しくは [開発記録](docs/DEVELOPMENT.md) と [検証記録](docs/VERIFICATION.md) を参照してください。AIが実装・資料確認・検証を担当し、樋口が目的、違和感、採用する方向を提示した共同開発です。108条件の検査はAIがPlaywrightで実行した画面操作の自動化で、本人の手動検査108回という意味ではありません。
...
- このアプリの開発では、8GB／16GBの実機での推論速度・日本語品質・省電力性・費用対効果は未測定です。別途の[local-llm-recordの実測](https://github.com/takashi357/local-llm-record/tree/main/measurements)は特定端末・モデル・設問の記録で、この診断全体の性能根拠にはしていません。
...
| `docs/verification/` | 開発時の記録と、`github-review-2026-09-20/`に分けた今回の再実行結果 |
